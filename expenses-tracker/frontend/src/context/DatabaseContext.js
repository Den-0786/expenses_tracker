  ...
  const saveNote = (note) => {
    return new Promise((resolve, reject) => {
      ...
      if (db.mock) {
        ...
        db.mockData.notes.unshift(newNote);
        resolve({ insertId: newNote.id });
        return;
      }
      ...
      db.transaction((tx) => {
        tx.executeSql(
          ...
          [note.title, note.body, note.created_at, note.updated_at],
          (_, result) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.transaction((tx) => {
        tx.executeSql(
          ...
          [note.title, note.body, note.updated_at, note.id],
          (_, result) => {
